...
def main():
    ...
    parser.add_argument(
        "--modules",
        help="Comma-separated list of modules to load (default: dns,dhcp,icmp,tcp,sip,capinfos)",
        default="dns,dhcp,icmp,tcp,sip,capinfos",
    )
    parser.add_argument(
        "--max-packets",
        type=int,
    ...
    try:
        ...
        modules = (
            args.modules.split(",")
            if args.modules
            else ["dns", "dhcp", "icmp", "tcp", "sip", "capinfos"]
        )
